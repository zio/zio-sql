...
trait SqlServerJdbcModule extends SqlServerRenderModule with Jdbc {
  ...
  override def setParam(param: SqlParameter, jdbcIndex: Int): java.sql.PreparedStatement => Unit = ps =>
    param._type match {
      ...
      case BinaryType         => ps.setBytes(jdbcIndex, param.value.asInstanceOf[Array[Byte]])
      ...
      case ZonedDateTimeType  =>
        ps.setString(jdbcIndex, fmtDateTimeOffset.format(param.value.asInstanceOf[ZonedDateTime]))
      case BigIntegerType     => ps.setLong(jdbcIndex, param.value.asInstanceOf[BigInt].longValue)
      case UUIDType           => ps.setString(jdbcIndex, param.value.asInstanceOf[UUID].toString())
