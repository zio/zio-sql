...
object SqlServerQueryShowSpec extends ZIOSpecDefault with SqlServerRenderModule {
  ...
  override def spec: Spec[TestEnvironment with Scope, Any] = suite("SqlServerQueryShow")(
    test("rendering select") {
      ...
      val expectedQuery =
        "SELECT TOP 5 order.id, products.name FROM products inner join order on order.product_id = products.id "
