...
trait SqlServerTestContainer extends JdbcRunnableSpec {
  ...
  override val poolConfigLayer: ZLayer[Any, Throwable, ConnectionPoolConfig] =
    ZLayer.scoped {
      ZIO.acquireRelease {
        ZIO.attemptBlocking {
          val c = new MSSQLServerContainer(
            dockerImageName = DockerImageName.parse("mcr.microsoft.com/mssql/server:2019-latest")
          ).configure { a =>
