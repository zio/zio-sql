...
trait Jdbc extends zio.sql.Sql with TransactionModule with ExecuteBuilderModule with JdbcInternalModule {
  trait SqlExecutor  {
  ...
  object SqlExecutor {
    val live: ZLayer[Blocking with Has[ConnectionPool], Nothing, Has[SqlExecutor]] =
      (for {
        blocking <- ZIO.service[Blocking.Service]
        pool     <- ZIO.service[ConnectionPool]
      } yield SqlExecutorLive(blocking, pool)).toLayer
  }
  sealed case class SqlExecutorLive(blocking: Blocking.Service, pool: ConnectionPool) extends SqlExecutor {
    def delete(delete: Delete[_]): IO[Exception, Int] =
  ...
  def execute[R, E >: Exception, A](tx: ZTransaction[R, E, A]): ZIO[R with Has[SqlExecutor], E, A] =
    ZIO.accessM[R with Has[SqlExecutor]](_.get.transact(tx))

  def execute[A <: SelectionSet[_]](read: Read[A]): ExecuteBuilder[A, read.ResultType] =
    new ExecuteBuilder(read)

  def execute(delete: Delete[_]): ZIO[Has[SqlExecutor], Exception, Int] =
    ZIO.accessM[Has[SqlExecutor]](
      _.get.delete(delete)
    )
