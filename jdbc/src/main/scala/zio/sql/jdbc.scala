...
trait Jdbc extends Sql {
      ...
      ZLayer.fromFunctionManaged[Has[Config] with Blocking, IOException, Service] { env =>
        ...
        ZManaged.succeed {
          new Service {
            ...
            def connection: Managed[Exception, Connection] = 
              Managed.make(blocking.effectBlocking(DriverManager.getConnection(config.url, config.properties)).refineToOrDie[IOException])(conn => 
              blocking.effectBlocking(conn.close()).orDie)
          }
        }
      }
  }
  ...
  object ReadExecutor {
    ...
    val live: ZLayer[ConnectionPool with Blocking, Nothing, ReadExecutor] = 
      ZLayer.fromServices[ConnectionPool.Service, Blocking.Service, ReadExecutor.Service] { (pool, blocking) =>
        new Service {
          ...
          def execute[A <: SelectionSet[_], Target](read: Read[A])(to: read.ResultType => Target): Stream[Exception, Target] =
            ZStream.managed(pool.connection).flatMap(conn =>
              Stream.unwrap {             
    ...
    private[sql] def extractColumn[A](column: Either[Int, String], resultSet: ResultSet, typeTag: TypeTag[A], nonNull: Boolean = true): Either[DecodingError, A] = {
      import TypeTag._ 
      ...
      typeTag match {
        ...
        case TByteArray       => tryDecode[Chunk[Byte]](Chunk.fromArray(column.fold(resultSet.getBytes(_), resultSet.getBytes(_))))
        ...
        case TInstant         => tryDecode[java.time.Instant](column.fold(resultSet.getTimestamp(_), resultSet.getTimestamp(_)).toInstant())
        case TInt             => tryDecode[Int](column.fold(resultSet.getInt(_), resultSet.getInt(_)))
        case TLocalDate       => tryDecode[java.time.LocalDate](column.fold(resultSet.getTimestamp(_), resultSet.getTimestamp(_)).toLocalDateTime().toLocalDate())
        case TLocalDateTime   => tryDecode[java.time.LocalDateTime](column.fold(resultSet.getTimestamp(_), resultSet.getTimestamp(_)).toLocalDateTime())
        case TLocalTime       => tryDecode[java.time.LocalTime](column.fold(resultSet.getTimestamp(_), resultSet.getTimestamp(_)).toLocalDateTime().toLocalTime())
    ...
    private[sql] def unsafeExtractRow[A](resultSet: ResultSet, schema: Vector[(TypeTag[_], Int)]): Either[DecodingError, A] = {
      val result: Either[DecodingError, Any] = Right(())

      schema.foldRight(result) {
  ...
  class ExecuteBuilder[Set <: SelectionSet[_], Output](val read: Read.Aux[Output, Set]) {
    ...
    def to[A, B, Target](f: (A, B) => Target)(implicit ev: Output <:< (A, (B, Unit))): ZStream[ReadExecutor, Exception, Target] = 
      ZStream.unwrap(ZIO.access[ReadExecutor](_.get.execute(read) { resultType => 
