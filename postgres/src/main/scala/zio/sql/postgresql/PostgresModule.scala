...
import java.time._
import zio.sql.Jdbc
import zio.sql.rendering.Builder

import scala.language.implicitConversions

/**
 */
trait PostgresModule extends Jdbc with PostgresRendering[_] { self =>
  ...
  override def renderRead(read: self.Read[_]): String = {
    ...
    PostgresRendering.renderReadImpl(read)
  ...
  def renderUpdate(update: Update[_]): String = {
    implicit val render: Builder = Builder()
    PostgresRendering.renderUpdateImpl(update)
    println(render.toString)
    render.toString
  }
  ...
  override def renderDelete(delete: Delete[_]): String = {
    implicit val render: Builder = Builder()
    PostgresRendering.renderDeleteImpl(delete)
    println(render.toString)
    render.toString
  }
